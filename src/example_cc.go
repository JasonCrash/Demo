...
// a transaction struct similar to the marble with the type expansion mapping as:
// ObjectType:
// Name:       id
// coulor:     untildate
// Size:      pointID
...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "invoke" { //get marbles based on range query
		return t.invoke(stub, args)
	} else if function == "query" { //get marbles based on range query
...
func (t *SimpleChaincode) initMarble(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	size3, err := strconv.Atoi(args[5])
	owner := strings.ToLower(args[6])
	orderdate := strings.ToLower(args[7])
	user := strings.ToLower(args[8])
	...
	objectType := "marble"
	marble := &marble{objectType, marbleName, color, size,size1,size2,size3,owner,orderdate,user}
	...
	indexName := "user~name"
	colorNameIndexKey, err := stub.CreateCompositeKey(indexName, []string{marble.User, marble.Name})
	if err != nil {
		return shim.Error(err.Error())
	}
...
func (t *SimpleChaincode) delete(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	// maintain the index
	indexName := "user~name"
	colorNameIndexKey, err := stub.CreateCompositeKey(indexName, []string{marbleJSON.User, marbleJSON.Name})
	if err != nil {
		return shim.Error(err.Error())
	}
